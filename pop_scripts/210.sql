                                 ...
                                 END) * (CASE
                                             WHEN be.oneortwo = 2 THEN (CASE
                                                                            WHEN (grm.component = 'SURVIVOR'
                                                                                  ...
                                                                                  OR grm.component LIKE 'REVERSION%') THEN (tree.volcsnet / plot.remper)
                                                                            WHEN (grm.component LIKE 'CUT%'
                                                                                  OR grm.component LIKE 'DIVERSION%') THEN (tre_midpt.volcsnet / plot.remper)
                                                                            ELSE (0)
                                                                        END)
                                             ELSE (CASE
                                                       WHEN (grm.component = 'SURVIVOR'
                                                             ...
                                                             OR grm.component = 'MORTALITY1') THEN CASE
                                                                                                       WHEN tre_begin.tre_cn IS NOT NULL THEN - (tre_begin.volcsnet / plot.remper)
                                                                                                       ELSE - (ptree.volcsnet / plot.remper)
                                                                                                   END
                                                       ELSE (0)
                                                   END)
