                                 ...
                                 END) * (CASE
                                             WHEN be.oneortwo = 2 THEN (CASE
                                                                            WHEN (grm.component = 'SURVIVOR'
                                                                                  ...
                                                                                  OR grm.component LIKE 'REVERSION%') THEN ((tree.drybio_bole + tree.drybio_bole_bark) / 2000 / plot.remper - (tree.drybio_sawlog + tree.drybio_sawlog_bark) / 2000 / plot.remper)
                                                                            WHEN (grm.component LIKE 'CUT%'
                                                                                  OR grm.component LIKE 'DIVERSION%'
                                                                                  OR grm.component LIKE 'MORTALITY%') THEN ((tre_midpt.drybio_bole + tre_midpt.drybio_bole_bark) / 2000 / plot.remper - (tre_midpt.drybio_sawlog + tre_midpt.drybio_sawlog_bark) / 2000 / plot.remper)
                                                                            ELSE (0)
                                                                        END)
                                             ELSE (CASE
                                                       WHEN (grm.component = 'SURVIVOR'
                                                             ...
                                                             OR grm.component = 'MORTALITY1') THEN CASE
                                                                                                       WHEN tre_begin.tre_cn IS NOT NULL THEN - ((tre_begin.drybio_bole + tre_begin.drybio_bole_bark) / 2000 / plot.remper - (tre_begin.drybio_sawlog + tre_begin.drybio_sawlog_bark) / 2000 / plot.remper)
                                                                                                       ELSE - ((ptree.drybio_bole + ptree.drybio_bole_bark) / 2000 / plot.remper - (ptree.drybio_sawlog + ptree.drybio_sawlog_bark) / 2000 / plot.remper)
                                                                                                   END
                                                       ELSE (0)
                                                   END)
