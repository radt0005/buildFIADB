                                 ...
                                 END) * (CASE
                                             WHEN be.oneortwo = 2 THEN (CASE
                                                                            WHEN (grm.component = 'SURVIVOR'
                                                                                  ...
                                                                                  OR grm.component LIKE 'REVERSION%') THEN ((coalesce(tree.voltssnd, 0) + coalesce(tree.voltssnd_bark, 0))/plot.remper)
                                                                            WHEN (grm.component LIKE 'CUT%'
                                                                                  OR grm.component LIKE 'DIVERSION%') THEN ((coalesce(tre_midpt.voltssnd, 0) + coalesce(tre_midpt.voltssnd_bark, 0))/plot.remper)
                                                                            ELSE (0)
                                                                        END)
                                             ELSE (CASE
                                                       WHEN (grm.component = 'SURVIVOR'
                                                             ...
                                                             OR grm.component = 'MORTALITY1') THEN CASE
                                                                                                       WHEN tre_begin.tre_cn IS NOT NULL THEN -((coalesce(tre_begin.voltssnd, 0) + coalesce(tre_begin.voltssnd_bark, 0))/plot.remper)
                                                                                                       ELSE -((coalesce(ptree.voltssnd, 0) + coalesce(ptree.voltssnd_bark, 0))/plot.remper)
                                                                                                   END
                                                       ELSE (0)
                                                   END)
...
LEFT OUTER JOIN
  (SELECT tre_cn,
          ...
          dia_midpt,
          dia_end,
          micr_component_al_timber AS component,
          micr_subptyp_grm_al_timber AS subptyp_grm,
          micr_tpagrow_unadj_al_timber AS tpagrow_unadj
   FROM fs_fiadb.tree_grm_component) grm ON (tree.cn = grm.tre_cn)
JOIN fs_fiadb.ref_species ON (tree.spcd = ref_species.spcd)
WHERE ref_species.woodland = 'Y'
