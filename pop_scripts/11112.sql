                                 ...
                                 END) * (CASE
                                             WHEN be.oneortwo = 2 THEN (CASE
                                                                            WHEN (grm.component = 'SURVIVOR'
                                                                                  ...
                                                                                  OR grm.component LIKE 'REVERSION%') THEN (coalesce(tree.voltssnd + tree.voltssnd_bark, 0)/plot.remper)
                                                                            WHEN (grm.component LIKE 'CUT%'
                                                                                  OR grm.component LIKE 'DIVERSION%'
                                                                                  OR grm.component LIKE 'MORTALITY%') THEN (coalesce(tre_midpt.voltssnd + tre_midpt.voltssnd_bark, 0)/plot.remper)
                                                                            ELSE (0)
                                                                        END)
                                             ELSE (CASE
                                                       WHEN (grm.component = 'SURVIVOR'
                                                             ...
                                                             OR grm.component = 'MORTALITY1') THEN CASE
                                                                                                       WHEN tre_begin.tre_cn IS NOT NULL THEN -(coalesce(tre_begin.voltssnd + tre_begin.voltssnd_bark, 0)/plot.remper)
                                                                                                       ELSE -(coalesce(ptree.voltssnd + ptree.voltssnd_bark, 0)/plot.remper)
                                                                                                   END
                                                       ELSE (0)
                                                   END)
