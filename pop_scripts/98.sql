SELECT peg.statecd,
       peg.eval_grp,
       98 attribute_nbr,
       sum((0.90718474 * cond.condprop_unadj * (coalesce(cond.carbon_understory_ag, 0)) * CASE cond.prop_basis
                                                                                              WHEN 'MACR' THEN pop_stratum.adj_factor_macr
                                                                                              ELSE pop_stratum.adj_factor_subp
                                                                                          END + coalesce((
                                                                                                            (SELECT sum(0.90718474 * CASE treesq.statuscd
                                                                                                                                         WHEN 1 THEN 1
                                                                                                                                         ELSE 0
                                                                                                                                     END * treesq.tpa_unadj * (coalesce(treesq.carbon_ag, 0)) / 2000 * CASE
