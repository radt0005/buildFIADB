                                 ...
                                 END) * (CASE
                                             WHEN be.oneortwo = 2 THEN (CASE
                                                                            WHEN (grm.component = 'SURVIVOR'
                                                                                  ...
                                                                                  OR grm.component LIKE 'REVERSION%') THEN (((tree.drybio_stem + tree.drybio_stem_bark) - (tree.drybio_bole + tree.drybio_bole_bark) - (tree.drybio_stump + tree.drybio_stump_bark) + tree.drybio_branch) / 2000 / plot.remper)
                                                                            WHEN (grm.component LIKE 'CUT%'
                                                                                  OR grm.component LIKE 'DIVERSION%') THEN (((tre_midpt.drybio_stem + tre_midpt.drybio_stem_bark) - (tre_midpt.drybio_bole + tre_midpt.drybio_bole_bark) - (tre_midpt.drybio_stump + tre_midpt.drybio_stump_bark) + tre_midpt.drybio_branch) / 2000 / plot.remper)
                                                                            ELSE (0)
                                                                        END)
                                             ELSE (CASE
                                                       WHEN (grm.component = 'SURVIVOR'
                                                             ...
                                                             OR grm.component = 'MORTALITY1') THEN CASE
                                                                                                       WHEN tre_begin.tre_cn IS NOT NULL THEN - (((tre_begin.drybio_stem + tre_begin.drybio_stem_bark) - (tre_begin.drybio_bole + tre_begin.drybio_bole_bark) - (tre_begin.drybio_stump + tre_begin.drybio_stump_bark) + tre_begin.drybio_branch) / 2000 / plot.remper)
                                                                                                       ELSE - (((ptree.drybio_stem + ptree.drybio_stem_bark) - (ptree.drybio_bole + ptree.drybio_bole_bark) - (ptree.drybio_stump + ptree.drybio_stump_bark) + ptree.drybio_branch) / 2000 / plot.remper)
                                                                                                   END
                                                       ELSE (0)
                                                   END)
...
LEFT OUTER JOIN
  (SELECT tre_cn,
          ...
          dia_midpt,
          dia_end,
          subp_component_sl_forest AS component,
          subp_subptyp_grm_sl_forest AS subptyp_grm,
          subp_tpagrow_unadj_sl_forest AS tpagrow_unadj
