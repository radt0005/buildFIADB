                                 ...
                                 END) * (CASE
                                             WHEN be.oneortwo = 2 THEN (CASE
                                                                            WHEN (grm.component = 'SURVIVOR'
                                                                                  ...
                                                                                  OR grm.component LIKE 'REVERSION%') THEN (tree.volbfnet / plot.remper)
                                                                            WHEN (grm.component LIKE 'CUT%'
                                                                                  OR grm.component LIKE 'DIVERSION%'
                                                                                  OR grm.component LIKE 'MORTALITY%') THEN (tre_midpt.volbfnet / plot.remper)
                                                                            ELSE (0)
                                                                        END)
                                             ELSE (CASE
                                                       WHEN (grm.component = 'SURVIVOR'
                                                             ...
                                                             OR grm.component = 'MORTALITY1') THEN CASE
                                                                                                       WHEN tre_begin.tre_cn IS NOT NULL THEN - (tre_begin.volbfnet / plot.remper)
                                                                                                       ELSE - (ptree.volbfnet / plot.remper)
                                                                                                   END
                                                       ELSE (0)
                                                   END)
