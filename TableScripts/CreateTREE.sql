...
comment on column fs_fiadb.tree.prev_pntn_srs
  is 'SRS is probably going to add past periodic prism trees into NIMS and FIADB. In some older SE states, the prism point, tree number (PNTN) of the current cycle did not match the previous cycles prism point, tree number. To be able to join the current and the previous prism plot trees, we need to have the PREV_PNTN_SRS assigned to the current tree record.';
...
alter table FS_FIADB.TREE add constraint TRE_UK unique (TREE, SUBP, PLT_CN);
